...
class AttModuleDir(nn.Module):
    ...
    def forward(self, x, input, flow_idx, HS = None):
        xsize = x.size()
        sample_num = xsize[0]
        ...
        if flow_idx == 1:
            value_probe, HS_probe = self.selfpooling_model(probe_x, probe_input,singleflow=False)
            value_gallery, HS_gallery = self.selfpooling_model(gallery_x, gallery_input,singleflow=False)
            return value_probe, value_gallery, [HS_probe, HS_gallery]
        elif flow_idx == 0 :
            pooled_probe,_ = self.selfpooling_model(probe_x, probe_input,singleflow=False, Hs=HS[0])
            pooled_gallery,_ = self.selfpooling_model(gallery_x, gallery_input,singleflow=False, Hs=HS[1])
        else:
            # only one flow
            pooled_probe, hidden_probe = self.selfpooling_model(probe_x, probe_input)
            pooled_gallery, hidden_gallery = self.selfpooling_model(gallery_x, gallery_input)
